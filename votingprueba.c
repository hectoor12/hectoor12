...
void candidato(int n_procs, FILE *archivo, char *votos, sigset_t oset) {
...
void votante(int n_procs, FILE *v, sigset_t oset) {
    char voto;

    sigsuspend(&oset);
    
    voto = decideYN(getpid());
    fprintf(v, "%c ", voto);
        ...
        for (i = 0; i < n_procs; i++) {
            ...
            if (pid == 0) {
                ...
                if (sem_trywait(sem_candidato) == 0) {
                    ...
                    fclose(v);
                    candidato(n_procs, f, votos, oset);
                    fclose(f);
                    exit(EXIT_SUCCESS);
                } else {
                    fclose(f);
                    sem_close(sem_candidato);
                    votante(n_procs, v, oset);
                    fclose(v);
                    exit(EXIT_SUCCESS);
                }
            } else {
